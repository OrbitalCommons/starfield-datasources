...
fn augment_missing_respects_mag_limit() {
    let mut cat_full = Dr2Catalog::new();
    let n_full = cat_full.augment_missing(f64::INFINITY).unwrap();

    let mut cat_bright = Dr2Catalog::new();
...
#[test]
fn every_inserted_entry_has_supplement_source_id_and_dr2_epoch() {
    let mut cat = Dr2Catalog::new();
